/**
 * The program asks for user inputs of binary or integer string and 
 * check whether that is a valid and not empty string of binary or integer. 
 * If it is a valid string, the program transform that string from Binary to Integer or vice versa depending on user's choice.
 * @author Linh Nguyen
 * @version 10232019
 */
...
public class BinaryNumbers {
    ...
    public static boolean validInteger(String value) {
        ...
        if (value.length()==0) {// deal with empty string
        	validInt = false;
        }
        else {
        	for (int i =0; i < value.length(); ++i) {
        		...
        		if (!Character.isDigit(c)) {
        			validInt = false; // assume the String contains valid integer and detect false cases only - not sure this is the best way to do it
        		}
        	}
        }
        return validInt;
    ...
    public static boolean validBinary(String value) {
        ...
        if (value.length()==0) {// deal with empty string
        	...
        	for (int i = 0; i< value.length();++i) {
        		char c = value.charAt(i);
        		if ( c != '1' && c != '0') { // check if the character is 1 or 0
        				validBin = false; // assume the Binary String is valid and detect the false case only. Not sure this is the best way to do it
          		}
        	}
        }
        return validBin;
    ...
    public static int binaryToInt(String value) {
    	int resultInt = 0;
    	int pow = 1;
    	for (int i = value.length()-1 ; i >= 0; --i) {// implement the algorithm from right to left 
    		char c = value.charAt(i);
    		int val = Character.getNumericValue(c);
    		int multiple = val * pow;
