...
public class PokerDice {
    ...
    public static void main(String[] args) {
		...
		int count =0; // count the number of play to deal with format
		while (!stop) {
			...
			else {// deal with format AGAIN
				msg = "Would you like to play again [Y/N]? ";
			}
			// prompt to ask whether the user wants to play
			...
			if (Play == true) {
				...
				// prompt to reroll
				promptForReroll(roll, keyboard);

				// final roll based on the rerolls the user wants
				rollDice(rnd, roll);

				System.out.println("Your final roll: " + Arrays.toString(roll));
				// decide
				System.out.println(scoreDice(roll));
				count ++;
			}
