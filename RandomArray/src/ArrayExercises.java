/**
 * The program asks for user' input and fill in 2 arrays with random numbers from 1 to 6. Then concatenate the array into a merged array with content interleaved.
 * @author Linh Nguyen
...
public class ArrayExercises {
    ...
    public static int[] mergeArrays(int[] arr1, int[] arr2) {
    	int [] arrayMerged = new int[arr1.length+ arr2.length];
    	int index1 = 0;
    	int index2 = 0;
    	for (int i = 0; i < arrayMerged.length; ++i) {
    		if (i%2==0) {
    			arrayMerged[i]=arr1[index1];
    			index1++;
    		}
    		else {
    			arrayMerged[i]=arr2[index2];
    			index2++;
    		}
    	}
