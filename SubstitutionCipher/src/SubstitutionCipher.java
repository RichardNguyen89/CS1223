/**
 * The program asks for user's choice of decoding or encoding a text file and write the result into another file.
 * @author Linh Nguyen
 * @version 11142019
 */
...
public class SubstitutionCipher {
	...
	while (input.length() == 0) { // deal with input length 0
	...
	try {
        	...
        	PrintWriter textWriter = new PrintWriter(writeFile); //open new file
	...
	while (!(input.length() == 1 && 
			(Character.toUpperCase(input.charAt(0))== 'E' || 
			Character.toUpperCase(input.charAt(0)) == 'D' || 
			Character.toUpperCase(input.charAt(0)) == 'Q'))) {// loop until condition is reached
    ...
    public static void main(String[] args) {
        ...
        while (!stop) { // loop til stop
        	displayMenu();
        	char choice = getChoice(in);
        	if (choice == 'D' || choice == 'E') {// check choice = D or E
        		
        		String inA = promptForString(in, msg1);
        		String outA = promptForString(in, msg2);
        		String shiftA = promptForString(in, msg3);
        		int shift1 = Integer.parseInt (shiftA);
        		transformFile(inA, outA, shift1); // transform file.
